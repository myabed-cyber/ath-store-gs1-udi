...
// Seed demo accounts (change via env)
const ADMIN_USER = process.env.SEED_ADMIN_USER || "admin";
const ADMIN_PASS = process.env.SEED_ADMIN_PASS || "admin123";
const OP_USER = process.env.SEED_OPERATOR_USER || "operator";
const OP_PASS = process.env.SEED_OPERATOR_PASS || "op123";
...
async function seed() {
  ...
  if (u.rows[0].c > 0) return;

  await q(
    "INSERT INTO users (id, username, password_hash, role, is_active) VALUES ($1,$2,$3,$4,true)",
    [uuid(), ADMIN_USER, bcrypt.hashSync(ADMIN_PASS, 10), "admin"]
  );
  await q(
    "INSERT INTO users (id, username, password_hash, role, is_active) VALUES ($1,$2,$3,$4,true)",
    [uuid(), OP_USER, bcrypt.hashSync(OP_PASS, 10), "operator"]
  );

  const policy = {
    expiry_required: true,
    tracking_policy: "LOT_ONLY",
    missing_gs_behavior: "BLOCK", // BLOCK | LOOKAHEAD
    accept_numeric_as_gtin: true,
    enforce_gtin_checkdigit: true,
    near_expiry_threshold_days: 90,
    near_expiry_severity: "WARN",
    allow_commit_on_warn: true, // Pilot default
  };
  await q(
    "INSERT INTO policies (id, name, version, is_active, config, created_at) VALUES ($1,$2,$3,$4,$5, now())",
    [uuid(), "Medical Production", 1, true, policy]
  );

  console.log("Seeded users + active policy.");
}
